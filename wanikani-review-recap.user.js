...
"use strict";
(() => {
  // src/wanikani/config.ts
  var PANEL_WIDTH = "clamp(280px, 20%, 460px)";
  var STORAGE_KEY = "wk-review-recap:v1";
  var LEVEL_STORAGE_KEY = "wk-review-recap:current-level";
  var EXPIRY_MS = 12 * 60 * 60 * 1e3;
  var MAX_WRONG_PER_TYPE = 12;
  var ITEM_INFO_FONT_SCALE = 1.15;
  var WK_FONT_SIZES = {
    xxsmall: 11,
    xsmall: 14,
    small: 16,
  ...
  var QUESTION_TYPES = ["meaning", "reading"];

  // src/wanikani/state.ts
  var session = {
  ...
  function load() {
    try {
      ...
      for (const record of Object.values(session.store.items)) {
        if (!record.resolved) {
  ...
  function meaningsOf(record) {
    ...
    const alternative = record.meanings.filter((m) => m.kind === "alternative").map((m) => m.text);
  ...
  function card(record, types) {
    ...
    const characterNode = record.image ? el("img", { class: "wkrr-card__image", src: record.image, alt: primary[0] || "radical" }) : el("span", { class: "wkrr-card__chars", lang: "ja", text: record.characters || "?" });
    const head = el(
      "div",
      ...
      el(
        ...
        showMeaning && record.counts.meaning > 0 && el("span", {
          class: "wkrr-card__count",
          title: "Wrong meaning answers",
          text: "M ×" + record.counts.meaning
        }),
        showReading && record.counts.reading > 0 && el("span", {
          class: "wkrr-card__count",
    ...
    if (showMeaning && (primary.length || alternative.length)) {
      rows.push(
        el(
          ...
          alternative.length ? el("span", { class: "wkrr-meaning-alt", text: " " + alternative.join(", ") }) : null
        )
      );
    }
  ...
  function exportText() {
    ...
    for (const record of sortedRecords()) {
      ...
      const readings = readingGroupsOf(record).map((g) => (g.label ? g.label + ": " : "") + g.readings.map((r) => r.text).join("、")).join(" | ");
      lines.push(
        "- " + (record.characters || primary[0] || "?") + " (" + (TYPE_LABEL[record.type] || record.type) + (outstanding.length ? ", still failing " + outstanding.join(" + ") : "") + ")"
      );
      if (failedTypes(record).includes("meaning")) {
        lines.push(
          "  meaning: " + primary.join(", ") + (alternative.length ? " [" + alternative.join(", ") + "]" : "")
        );
  ...
  // src/wanikani/counts.ts
  var COUNT_SELECTOR = ".lesson-and-review-count__count, .count-bubble";
  var LESSONS_SELECTOR = ".todays-lessons-widget";
  var DAY_PARAM = "utc_time_at_start_of_day";
  var lastCountHour = (/* @__PURE__ */ new Date()).getHours();
  var lastCountDate = (/* @__PURE__ */ new Date()).toDateString();
  ...
  function ensureCurrentLevel() {
    ...
    const label = [...widget.querySelectorAll(".wk-button__text")].map((node) => (node.textContent || "").trim()).find((text) => /^Level \d+$/.test(text));
    const level = label ? Number(label.replace(/\D+/g, "")) : 0;
    if (!level || currentLevel && currentLevel.level === level) return;
    const paths = [...widget.querySelectorAll("a.subject-srs-progress[href]")].map((anchor) => subjectPath(anchor.getAttribute("href") || "")).filter((path) => !!path);
  ...
  // src/wanikani/srs.ts
  var BURN_FROM_STAGE = 8;
  var SRS_BLOB_SELECTOR = 'script[type="application/json"][data-quiz-queue-target="subjectIdsWithSRS"]';
  var srsStages = null;
  var srsSource = "";
  ...
  // src/wanikani/peek.ts
  function buildPeek() {
    const record = quiz.subject;
    if (!record) return el("div");
    const color = TYPE_COLOR[record.type] || "#8a8a8a";
    const blocked = wouldBurn(record);
    const image = record.image ? el("img", { class: "wkrr-peek__image", src: record.image, alt: "" }) : null;
    const characterNode = image ?? el("span", { class: "wkrr-peek__chars", lang: "ja", text: record.characters || "?" });
    const head = el(
      "div",
...
`;
  ...
  // src/wanikani/progress.ts
  var PROGRESS_STORAGE_KEY = "wk-review-recap:progress";
  var PROGRESS_MAX_AGE_MS = 10 * 60 * 1e3;
  var PROGRESS_STAGES = [
  ...
  function refreshProgress() {
    ...
    try {
      ...
      wkof.ready("ItemData").then(() => wkof.ItemData.get_items("assignments")).then((items) => {
  ...
  function buildProgress(tally) {
    ...
    const summary = `${progressShare(tally, tally.total - locked)} unlocked · ${progressShare(tally, burned)} burned`;
  ...
  // src/wanikani/styles/panel.css.ts
  var scaled = (name) => Math.round(WK_FONT_SIZES[name] * ITEM_INFO_FONT_SCALE);
  var CSS = `
...
`;
  ...
  // src/wanikani/styles/dark-theme.css.ts
  var DARK_THEME_CSS = String.raw`
:root {
